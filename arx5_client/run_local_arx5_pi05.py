#!/usr/bin/env python3
"""
ARX5 local execution with dexbotic Pi05 remote inference.

Based on run_local_arx5_spirit.py, adapted for dexbotic Pi05 model:
- Uses DEFAULT image_type_to_camera mapping (matching dexbotic training data)
- Default image_size 728x728 (Pi05 model.process_images handles internal resizing)
- Default duration 0.1s (50-step trajectory from Pi05)

Server: playground/place_shoes_pi05_ws_server.py (GPU machine)
Client: this file (robot machine with arm + cameras)

Usage
-----
  python run_local_arx5_pi05.py \\
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Dexbotic Pi05 remote inference client for local ARX5 execution",
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="Camera names to rotate 180° (e.g. side wrist front)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Save inference input/output to dir (no arm execution)",
    )
    ...
    interface = LocalRobotInterface(
        ...
        image_type_to_camera=PI05_IMAGE_TYPE_TO_CAMERA,
    ...
    logger.info(
        "Starting Pi05 local control loop (task=%s, image_size=%s, safe_mode=%s, flip_cameras=%s, record_dir=%s)",
        args.task_name,
        image_size,
        safe_mode,
